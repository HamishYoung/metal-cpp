...
_NS_INLINE void MTK::View::setDelegate( const MTK::ViewDelegate* pDelegate )
{
	...
	{
		NS::Object::sendMessage< void >( this, _MTK_PRIVATE_SEL( setDelegate_ ), nullptr );
		objc_setAssociatedObject( this, _NS_DELEGATE_ASSOCIATION_KEY, nullptr, OBJC_ASSOCIATION_ASSIGN );
		return;
	}

	NS::Value* pWrapper = NS::Value::value( pDelegate );
	objc_setAssociatedObject( this, _NS_DELEGATE_ASSOCIATION_KEY, pWrapper, OBJC_ASSOCIATION_COPY );

	// drawInMTKView:

	void (*drawDispatch)( MTK::View*, SEL, id ) = []( MTK::View* pSelf, SEL _cmd, id pMTKView ){
		auto pDel = pSelf->delegate();
		pDel->drawInMTKView( (MTK::View *)pMTKView );
	};

	class_addMethod( (Class)_MTK_PRIVATE_CLS( MTKView ), _MTK_PRIVATE_SEL( drawInMTKView_ ), (IMP)drawDispatch, "v@:@" );

	// mtkView:drawableSizeWillChange:

	void (*drawableSizeWillChange)( MTK::View*, SEL, View*, CGSize ) = []( MTK::View* pSelf, SEL, View* pMTKView, CGSize size){
		auto pDel = pSelf->delegate();
	...
	class_addMethod( (Class)_MTK_PRIVATE_CLS( MTKView ), _MTK_PRIVATE_SEL( drawableSizeWillChange_ ), (IMP)drawableSizeWillChange, cbparams );

	Object::sendMessage< void >( this, _MTK_PRIVATE_SEL( setDelegate_ ), this );
}

_NS_INLINE MTK::ViewDelegate* MTK::View::delegate() const
{
	NS::Value* pWrapper = ( NS::Value* )(objc_getAssociatedObject((id)this, _NS_DELEGATE_ASSOCIATION_KEY));
	if(!pWrapper)
	{
