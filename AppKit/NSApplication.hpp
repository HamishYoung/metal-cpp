...
_NS_INLINE void NS::Application::setDelegate( const ApplicationDelegate* pAppDelegate )
{
    ...
    objc_setAssociatedObject( this, "delegate_ptr", pWrapper, OBJC_ASSOCIATION_COPY );

    // Dispatch functions
    typedef void (*VoidDispatchFunction)( NS::Application*, SEL, void* );
    typedef bool (*BoolDispatchFunction)( NS::Application*, SEL, void* );

    VoidDispatchFunction willFinishLaunching = []( NS::Application* pSelf, SEL, void* pNotification ){
        NS::Value* pDelegatePtr = ( NS::Value* )(objc_getAssociatedObject(pSelf, "delegate_ptr"));
        auto pDel = reinterpret_cast< NS::ApplicationDelegate* >( pDelegatePtr->pointerValue() );
        pDel->applicationWillFinishLaunching( (NS::Notification *)pNotification );
    };

    VoidDispatchFunction didFinishLaunching = []( NS::Application* pSelf, SEL, void* pNotification ){
        NS::Value* pDelegatePtr = ( NS::Value* )(objc_getAssociatedObject(pSelf, "delegate_ptr"));
        auto pDel = reinterpret_cast< NS::ApplicationDelegate* >( pDelegatePtr->pointerValue() );
        pDel->applicationDidFinishLaunching( (NS::Notification *)pNotification );
    };

    BoolDispatchFunction shouldTerminateAfterLastWindowClosed = []( NS::Application* pSelf, SEL, void* pApplication ){
        NS::Value* pDelegatePtr = ( NS::Value* )(objc_getAssociatedObject(pSelf, "delegate_ptr"));
        auto pDel = reinterpret_cast< NS::ApplicationDelegate* >( pDelegatePtr->pointerValue() );
        return pDel->applicationShouldTerminateAfterLastWindowClosed( (NS::Application *)pApplication );
    };

    class_addMethod( (Class)_APPKIT_PRIVATE_CLS( NSApplication ), _APPKIT_PRIVATE_SEL( applicationWillFinishLaunching_ ), (IMP)willFinishLaunching, "v@:@" );
    class_addMethod( (Class)_APPKIT_PRIVATE_CLS( NSApplication ), _APPKIT_PRIVATE_SEL( applicationDidFinishLaunching_ ), (IMP)didFinishLaunching, "v@:@" );
    class_addMethod( (Class)_APPKIT_PRIVATE_CLS( NSApplication ), _APPKIT_PRIVATE_SEL( applicationShouldTerminateAfterLastWindowClosed_), (IMP)shouldTerminateAfterLastWindowClosed, "B@:@" );

    Object::sendMessage< void >( this, _APPKIT_PRIVATE_SEL( setDelegate_ ), this );
}
